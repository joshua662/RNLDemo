...
const DeleteUserFormModal: FC<DeleteUserFormModalProps> = ({user, onUserDeleted, refreshKey, isOpen, onClose}) => {
   const [loadingDestroy, setLoadingDestroy] = useState(false);
    ...
    const handleDestroyer = async (e: FormEvent) => {
      try {
        e.preventDefault()
       ...
       const res = await UserService.destroyUser(user!.user_id);
        ...
        if(res.status === 200) {
          ...
          console.error(
            "Unexpected status error occured during deleting user: ",
            res.status
          );
        }
      } catch (error) {
                ...
                <form onSubmit={handleDestroyer}>
                    ...
                    <div className="grid grid-cols-2 gap-4 border-b border-gray-100 mb-4">
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <div className="mb-4">
                                <label htmlFor="gender" className="text-black font-medium mb-2">Gender</label>
                              <p className="text-gray-500 font-medium">{gender}</p>
                            </div>
                        </div>
                        <div className="col-span-2 md:col-span-1">
                            <div className="mb-4">
                               <label htmlFor="birth_date" className="text-black font-medium mb-2">Birth Date</label>
